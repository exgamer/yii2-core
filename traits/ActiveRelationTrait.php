    ...
    public function populateRelation($name, &$primaryModels)
    {
        ...
        foreach ($primaryModels as $i => $primaryModel) {
            ...
            if ($primaryModel instanceof ActiveRecordInterface) {
                ...
                #добавлен хак для заполнения дополнительных атрибутов массива
                #из связонной таблицы свойств
                #core\models\v2\properties\table\IARWithTableProps
                $modelClass = $this->modelClass;
                if( method_exists($this->modelClass, 'getPrimaryClass') ) {
                    $pimaryClass = $modelClass::getPrimaryClass();
                    if(
                        method_exists($pimaryClass, 'getPropertiesRelation') 
                        && $name ==  $pimaryClass::getPropertiesRelation() 
                        && is_array($value)
                    ) {
                        foreach ($value as $item) {
                            $primaryModels[$i][$item['name']] = $item['value_number'];
                        }
                        unset($primaryModels[$i][$name]);
                    }
                }
                #конец хака
            }
